...
// WriteMovieCast upserts a movie, its cast members and the ACTED_IN edges
// between them in one UNWIND batch. Movie properties are refreshed because
// the movie endpoint is the authoritative source for them, and crawled_at
// marks the movie as expanded (see MovieCrawled).
func (s *Store) WriteMovieCast(ctx context.Context, m Movie, cast []CastEntry) error {
...
// WriteFilmography upserts a person and every movie they acted in. Title
// and date are only filled in ON CREATE (the movie endpoint owns them once
// fetched); rating, votes and poster are the same figures from either
// endpoint, so the latest write wins and a movie never loses its poster.
func (s *Store) WriteFilmography(ctx context.Context, p Person, credits []FilmCredit) error {
	rows := make([]map[string]any, 0, len(credits))
	...
	if _, err := s.run(ctx, cypher, params); err != nil {
		return fmt.Errorf("graph: write person %d filmography: %w", p.ID, err)
	}
	return nil
}
